...
module Statleboncoin
	def self.run( update_db = false )
		...
		do_analysis db, 'hypermotard', db[:motos].where(model: "hypermotard")
		...
		do_analysis db, 'f800gs', db[:motos].where(model: "bmw%20gs", cylindree: "800 cm3").where(Sequel.like(:titre, '%800%')) .exclude(Sequel.like(:titre, '%700%')).exclude(Sequel.like(:titre, '%650%'))
		...
		do_analysis db, '1190 adventure', db[:motos].where(model: '1190%20adventure')
	
		#Tiger 800
		puts 'Tiger'
		www_lookup  db, 'triumph', /tiger/, update_db 
		do_analysis db, 'tiger 800', db[:motos].where(model: 'triumph', cylindree: '800 cm3')
		do_analysis db, 'tiger 1050', db[:motos].where(model: 'triumph', cylindree: '1 050 cm3')
